...
class Review:
    ...
    def year(self, year):
        if isinstance(year, int) and year >= 2000:
            ...
            raise ValueError(
                "Year must be an integer greater that 2000"
            )
    ...
    def instance_from_db(cls, row):
        ...
        if review:
            #Ensure attributes match row values in case local instance ws modified
            review.year= row[1]
...
def reviews(self):
    ...
    return [] if not rows else [Review.instance_from_db(row) for row in rows]
